...
本 git 仓库符合标签 VimL 插件的目录结构。如果采用 vundle 或 pathogen 等插件管
理工具安装，按相应的插件管理规范安装即可。

例如若用 vundle，则需在 vimrc 中加入如下代码，然后运行 vundle 的安装插件命令：
```
Plugin 'lymslive/vnote'
```

若用 pathogen ，则将该仓库下载至 ~/.vim/bundle 目录即可：
```
$ cd ~/.vim/bundle
$ git clone https://github.com/lymslive/vnote.git
```

也可以手动安装，按上面的命令将仓库下载至 ~/.vim/bundle 目录后，在 vimrc 中手动
将其加入 rtp (vim 的运行时搜索路径)：
```
...
打开 Vim ，在命令行中输入 `:NoteNew` 命令，就会在合适的路径中生成一个 `.md` 文
件。随便写些什么，当作普通文本文件编辑，然后保存。当写了一定量日记之后，可用命
令 `:NoteList` 查看日记列表。更详细用法请继续参考以下教程。
...
作者自己所作的笔记也将托管到 git，地址：https://github.com/lymslive/notebook
纯文本的笔记本，只要不涉及隐私，托管 git 不用操心。一般只需上传保存 `d/` 子目
录下的日记文件。
